...
The method prescribes starting in the central column of the first row `i <- 1; j <- (n + 1) / 2` with number 1. After that, the fundamental movement for filling the squares is diagonally right and down `i <- i + 1; j <- j + 1` (or it can be right-up, left-down, or left-up), one step at a time. If it moves beyond the right boundary, continue from the left side `i <- i - n`; if it moves beyond the bottom, continue from the top `j <- j - n`. If a filled square is encountered, one move vertically up one square instead `i <- i - 2; j <- j - 1`, then continue as before.
